...
const update = async () => {
    ...
    try {
        const response = await axios.put(`/api/jobs/${form.id}`, job);
        ...
        toast.success("Error updating job!");
    }
};
...
onMounted(async () => {
    ...
    try {
        const response = await axios.get(`/api/jobs/${form.id}`);
